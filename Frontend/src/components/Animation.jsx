...
export const initSimpleBackground = (canvas) => {
  const ctx = canvas.getContext("2d");
  let particles = [];
  ...
  class Particle {
    constructor() {
      ...
      this.radius = Math.random() * 0.8 + 0.3; // 👈 small dots
    }
    ...
    draw() {
      ctx.beginPath();
      ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(0, 207, 255, 0.6)"; // 👈 soft color
      ctx.fill();
    }
  }
    ...
    particles.forEach((p, i) => {
      ...
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < connectionDistance) {
          ...
          ctx.strokeStyle = `rgba(0, 207, 255, ${opacity * 0.2})`; // 👈 subtle
          ctx.lineWidth = 0.3;
